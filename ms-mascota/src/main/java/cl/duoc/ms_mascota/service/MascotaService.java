...
public class MascotaService {
    ...
    public Mascota guardar(Mascota mascota) {
        ...
        if (!Boolean.TRUE.equals(duenoClient.existeDueno(mascota.getIdDueno()))) {
            ...
            throw new BadRequestException("No se puede registrar mascota: El Dueño con ID " + mascota.getIdDueno() + " no existe.");
        }
    ...
    public Mascota actualizar(Long id, Mascota datosNuevos) {
        ...
        Mascota existente = mascotaRepository.findById(id)
            .orElseThrow(() -> { 
                logger.warn("Mascota no encontrada para actualizar id={}", id);
                return new ResourceNotFoundException("Mascota no encontrada con el ID: " + id);
            });

       
        if (!existente.getIdDueno().equals(datosNuevos.getIdDueno())) {
            logger.info("Validando existencia de nuevo dueño id={}", datosNuevos.getIdDueno());
            if (!Boolean.TRUE.equals(duenoClient.existeDueno(datosNuevos.getIdDueno()))) {
                ...
                throw new BadRequestException("El nuevo dueño especificado con ID " + datosNuevos.getIdDueno() + " no existe.");
            }
        }
    ...
    public void eliminar(Long id) {
        ...
        if (!mascotaRepository.existsById(id)) {
            throw new ResourceNotFoundException("No se puede eliminar: Mascota no encontrada con el ID: " + id);
        }
